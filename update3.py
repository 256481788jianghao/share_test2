...
def read_sql_table(table_name):
    return pd.read_sql('select * from '+table_name,sql_conn,index_col='index')

def isExists(table_name):
    try:
        data = read_sql_table(table_name)
...
#get trade cal
stock_trade_cal_table_name = "trade_cal"
stock_trade_cal_info = None
stock_trade_cal_info_tmp = ts_api.trade_cal(start_date='20160101',end_date=today_str2)
stock_trade_cal_info_tmp.to_sql(stock_trade_cal_table_name,sql_conn,if_exists='replace')

stock_trade_cal_info = read_sql_table(stock_trade_cal_table_name)
print(stock_trade_cal_table_name+' finish')
#print(stock_trade_cal_info)
    ...
    if item.is_open == 1:
        ...
        if not isExists(stockdata_table_name):
            try:
                ...
                data.to_sql(stockdata_table_name,sql_conn)
                print(stockdata_table_name+' finish')
            except Exception as e:
                print(e)
        if not isExists(stockdata_adjust_table_name):
            try:
...
stock_base_info_table_name = "baseInfo"
stock_base_info = None
stock_base_info_tmp = ts_api.stock_basic(list_status='L')
stock_base_info_tmp.to_sql(stock_base_info_table_name,sql_conn,if_exists='replace')
stock_base_info = read_sql_table(stock_base_info_table_name)
print(stock_base_info_table_name+' finish')
